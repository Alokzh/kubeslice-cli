...
var (
	licenseVerificationFunc = LicenseVerification
	generateValuesFileFunc  = generateValuesFile
)
...
func InstallKubeSliceController(ApplicationConfiguration *ConfigurationSpecs) {
	...
	podVerificationFunc("Waiting for KubeSlice Controller Pods to be Healthy", cc.ControllerCluster, KUBESLICE_CONTROLLER_NAMESPACE)
	...
	if ApplicationConfiguration.Configuration.ClusterConfiguration.Profile != "" && ApplicationConfiguration.Configuration.ClusterConfiguration.Profile == ProfileEntDemo {
		...
		licenseVerificationFunc("Waiting for KubeSlice Trial License to be Ready", cc.ControllerCluster, KUBESLICE_CONTROLLER_NAMESPACE)
	}
...
func generateControllerValuesFile(cluster Cluster, hcConfig HelmChartConfiguration) {
	err := generateValuesFileFunc(kubesliceDirectory+"/"+controllerValuesFileName, &hcConfig.ControllerChart, fmt.Sprintf(controllerValuesTemplate+generateImagePullSecretsValue(hcConfig.ImagePullSecret), cluster.ControlPlaneAddress))
	if err != nil {
